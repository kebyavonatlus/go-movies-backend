...
// All returns all movies and error, if eny
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	query := fmt.Sprintf(`
	select id, title, description, year, release_date, rating, runtime, mpaa_rating,
...
func (m *DBModel) InsertMovie(movie Movie) error {
	...
	stmt := `insert into movies (title, description, year, release_date,
...
func (m *DBModel) UpdateMovie(movie Movie) error {
	...
	stmt := `update movies set title = $1, description = $2, year = $3, release_date = $4,
		runtime = $5, rating = $6, mpaa_rating = $7, updated_at = $8
		where id = $9`
...
func (m *DBModel) DeleteMovie(id int) error {
	...
	stmt := `delete from movie where id = $1`
